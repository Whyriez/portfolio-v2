...
import GithubActivity from "@/components/features/github/GithubActivity";
...
// --- Konfigurasi Cache ---
const CACHE_KEY_PROFILE = "profile_data_cache";
const CACHE_KEY_PROJECTS = "home_projects_cache"; // Key baru untuk Project Home
...
const DEFAULT_BIO = "I'm a dedicated Mobile Developer & Backend Engineer passionate about building seamless and powerful digital experiences.";
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    // --- 2. Logic Fetch Projects dengan Caching (BARU) ---
    const fetchProjectsWithCache = async () => {
      ...
      try {
        ...
        const featuredData = allData.slice(0, 3); // Ambil 3 teratas saja

        setProjectData(featuredData);

        // Simpan ke Cache (Hanya 3 data agar hemat storage)
        localStorage.setItem(CACHE_KEY_PROJECTS, JSON.stringify({
          data: featuredData,
          timestamp: now
        }));
