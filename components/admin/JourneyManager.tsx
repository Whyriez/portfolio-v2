...
export default function JourneyManager() {
  ...
  const [deletingId, setDeletingId] = useState<string | null>(null); // Untuk loading saat delete

  const fetchJourneys = async () => {
    try {
      const res = await fetch('/api/journeys');
      const data = await res.json();
      // Sort journey: Asumsi data terbaru di atas, atau bisa di sort manual disini
      setJourneys(data);
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // Update state langsung agar UI responsif tanpa fetch ulang
    ...
    <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-gray-700 h-full flex flex-col">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
           ...
           <p className="text-sm text-gray-500 dark:text-gray-400">Add your work experience timeline.</p>
        </div>
      </div>

      {/* Form Section */}
      <form onSubmit={handleSubmit} className="mb-10 bg-gray-50 dark:bg-white/5 p-6 rounded-2xl border border-gray-100 dark:border-white/10 relative overflow-hidden group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
          ...
          <div className="col-span-1">
             ...
             <input required placeholder="Short summary of what you did..." value={form.description} onChange={e => setForm({...form, description: e.target.value})} 
               className="w-full p-3 rounded-xl bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all text-sm" />
          </div>
          
          <div className="md:col-span-2 mt-2 flex justify-end">
             <button type="submit" disabled={isAdding} className="bg-gray-900 dark:bg-white text-white dark:text-gray-900 px-6 py-2.5 rounded-xl font-bold hover:opacity-90 transition-all active:scale-95 text-sm flex items-center gap-2">
          ...
          <div className="relative border-l-2 border-gray-100 dark:border-white/10 ml-3 space-y-8 pb-4">
            ...
            {journeys.map((item, index) => (
              <div key={item.id} className="relative ml-8 group">
                {/* Timeline Dot */}
                <div className="absolute -left-[41px] top-1 h-5 w-5 rounded-full border-4 border-white dark:border-gray-800 bg-blue-500 group-hover:scale-125 transition-transform shadow-md"></div>
                
                {/* Content Card */}
                <div className="bg-white dark:bg-white/5 p-4 rounded-2xl border border-gray-100 dark:border-white/5 shadow-sm hover:shadow-md transition-all flex justify-between items-start group-hover:border-blue-100 dark:group-hover:border-blue-500/30">
                  <div>
